    ...
    try:
        if len(sys.argv) > 1 and sys.argv[1] == "--config":
            ...
            run_app = inquirer.confirm(
                message="Do you want to run VAS now?", default=True
            ).execute()
            if run_app:
                os.system('cls')
            else:
    ...
    while True:
        ...
        if True:
                                ...
                                if curr_player_stat["match_id"] != coregame.match_id:
                                    ...
                                    if player["PlayerIdentity"]["Incognito"] == False:
                                        ...
                                        already_played_with.append(
                                            {
                                                "times": times,
                                                "name": agent_dict.get(player["CharacterID"].lower(), "Unknown") + " on " + team_string + " team",
                                                "agent": curr_player_stat["agent"],
                                                "time_diff": time.time() - curr_player_stat["epoch"]
                                            })
                ...
                with richConsole.status("Loading Players...") as status:
                    ...
                    for player in Players:
                        ...
                        if player["CharacterSelectionState"] == "locked":
                            agent_color = color(agent_dict.get(player["CharacterID"].lower(), "Unknown"),
                                                fore=(255, 255, 255))
                            ...
                            agent_color = color(
                                agent_dict.get(player["CharacterID"].lower(), "Unknown"), fore=(54, 53, 51))
            ...
            if title is not None:
                ...
                table.set_caption(f"ValoAssist v{version}")
